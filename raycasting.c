...
/*   Updated: 2023/08/02 14:22:46 by smuhamma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_raycast(t_info *info, int x)
{
	...
	info->raycast.rdir.x = info->raycast.dir.x + info->raycast.plane.x * info->raycast.camera;
	info->raycast.rdir.y = info->raycast.dir.y + info->raycast.plane.y * info->raycast.camera;
...
void	direction_ray(t_info *info)
{
	...
	{
		info->raycast.step.x = 1;
		info->raycast.rdist.x = (info->raycast.rmap.x + 1.0 - info->raycast.rpos.x) *
		info->raycast.rdisd.x;
	}
	...
	{
		info->raycast.step.y = 1;
		info->raycast.rdist.y = (info->raycast.rmap.y + 1.0 - info->raycast.rpos.y) *
			info->raycast.rdisd.y;
	}
}
...
void	size_ray(t_info *info)
{
	if (info->raycast.wall == 0 || info->raycast.wall == 2)
		...
		info->raycast.dist = (info->raycast.rmap.y - info->raycast.rpos.y
					+ (1 - info->raycast.step.y) / 2) / info->raycast.rdir.y;
...
void	wall_textur(t_info *info)
{
	...
	if (info->raycast.wall == 0 || info->raycast.wall == 2)
		wallx = info->raycast.rpos.y + info->raycast.dist * info->raycast.rdir.y;
	else
		wallx = info->raycast.rpos.x + info->raycast.dist * info->raycast.rdir.x;
	wallx -= floor((wallx));
	info->raycast.textur.x = (int)(wallx * 64.0);
	if ((info->raycast.wall == 0 || info->raycast.wall == 2) && info->raycast.rdir.x > 0)
		info->raycast.textur.x = 64 - info->raycast.textur.x - 1;
	if ((info->raycast.wall == 1 || info->raycast.wall == 3) && info->raycast.rdir.y < 0)
		info->raycast.textur.x = 64 - info->raycast.textur.x - 1;
	if ((info->raycast.wall == 0 || info->raycast.wall == 2) && info->raycast.rdir.x >= 0)
		info->raycast.textur.id = 1;
	else if ((info->raycast.wall == 0 || info->raycast.wall == 2) && info->raycast.rdir.x < 0)
		info->raycast.textur.id = 0;
	else if ((info->raycast.wall == 1 || info->raycast.wall == 3) && info->raycast.rdir.y < 0)
